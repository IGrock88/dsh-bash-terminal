    ...
    async execute(args, exec) {
      ...
      switch (args.action) {
        case "open": {
          ...
          // wsl.exe under ConPTY needs the core Windows variables - SystemRoot
          // above all - or it dies with Wsl/Service/0x8007072c. The shell-env
          // seam is a curated shell set and omits them, so the process
          // environment is the base layer and dshEnv still overrides it.
          const env = { ...process.env, NO_COLOR: "1", TERM: "dumb", PAGER: "cat", GIT_PAGER: "cat", ...dshEnv };
